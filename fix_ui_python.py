...
with open('src/components/AddMediaModal.tsx', 'r') as f:
    content = f.read()

# We want to find the whole block starting with <div\n                  onDragOver={handleDragOver}
# and ending at the corresponding </div> before </div>\n              </div>\n            ) : (

pattern = r'<div\s+onDragOver=\{handleDragOver\}[\s\S]*?\{fileUrl \? \([\s\S]*?<\/div>\s*\)\}\s*<\/div>'
...
with open('src/components/AddMediaModal.tsx', 'w') as f:
    f.write(new_content)
